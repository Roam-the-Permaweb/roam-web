...
class WayfinderService {
  ...
  constructor(config: Partial<WayfinderConfig> = {}) {
    ...
    this.config = validateConfig({ ...DEFAULT_CONFIG, ...persistedConfig, ...config })
    
    // Initialize modules
    this.cache = new ContentCache()
    this.verificationManager = new VerificationManager()

    logger.info('WayfinderService initialized with config:', this.config)
  }
  ...
  private async _doInitialize(): Promise<void> {
    try {
      ...
      if (this.config.verification.enabled) {
        ...
        logger.info('Verification disabled, skipping verification strategy creation')
      }
      ...
      const wayfinderConfig: any = {
        ...
        routingStrategy,
        verificationStrategy, // undefined when verification is disabled
        events, // empty object when verification is disabled
  ...
  private shouldAutoFetch(contentType: string, size: number): boolean {
    if (contentType.startsWith('image/') && size > SIZE_THRESHOLDS.IMAGE) return false
    if (contentType.startsWith('video/') && size > SIZE_THRESHOLDS.VIDEO) return false
    if (contentType.startsWith('audio/') && size > SIZE_THRESHOLDS.AUDIO) return false
    if (
    ...
    if (this.wayfinder && this.config.enableWayfinder) {
      try {
        ...
        if (!this.config.verification.enabled) {
          ...
          logger.info(`Verification disabled, using Wayfinder for routing only (no verification)`)
        ...
        if (classifiedError instanceof GatewayError) {
          logger.warn('AR.IO gateway network temporarily unavailable, using direct gateway')
        } else {
        ...
        if (this.config.verification.enabled) {
          const status = classifiedError instanceof GatewayError ? 'not-verified' : 'failed'
   ...
   */
  private async fetchWithoutVerification(request: ContentRequest): Promise<ContentResponse> {
    ...
    const gatewayUrl = await this.wayfinder!.resolveUrl({
      originalUrl: arUrl,
    })
   ...
   */
  private async fetchViaWayfinder(request: ContentRequest): Promise<ContentResponse> {
    const { txId, path = '' } = request
    const arUrl = `ar://${txId}${path}`
   ...
   */
  private async processResponseData(response: Response, contentType: string): Promise<Blob> {
    if (
  ...
  private createUrlOnlyResponse(
    ...
    const fallbackGateway = this.config.fallback.gateways[0] || determineFallbackGateway()
          ...
          return retryWithBackoff(
            () => originalGetGateways(),
            {
              ...
              onRetry: (error, attempt) => {
                logger.warn(`NetworkGatewaysProvider.getGateways attempt ${attempt} failed:`, error.message)
              },
            }
          )
        }
  ...
  private async createRoutingStrategy(strategy: string, gatewayProvider?: any) {
    ...
    switch (strategy) {
      ...
      case 'static':
        const staticGateway = this.config.routing.strategy.staticGateway || 'https://arweave.net'
      ...
      case 'round-robin':
        ...
        if (gatewayProvider) {
          try {
            ...
            logger.warn('Failed to get gateways from provider, using fallback:', error)
      ...
      default:
        logger.warn(`Unknown routing strategy: ${strategy}, falling back to random`)
        return new RandomRoutingStrategy()
    }
  }
  ...
  private async createVerificationStrategy() {
    if (!this.config.verification.enabled || this.config.verification.strategy === 'none') {
      return undefined
    }
    ...
    try {
      ...
      logger.info(`Verification gateways available: ${verificationGateways.length}`)
    } catch (error) {
  ...
  updateConfig(newConfig: Partial<WayfinderConfig>): void {
    ...
    this.config = validateConfig({
      ...
      routing: {
        ...this.config.routing,
        ...(newConfig.routing || {}),
        gatewayProvider: newConfig.routing?.gatewayProvider || this.config.routing.gatewayProvider,
        strategy: newConfig.routing?.strategy || this.config.routing.strategy,
      },
      verification: {
        ...this.config.verification,
        ...(newConfig.verification || {}),
        gatewayProvider: newConfig.verification?.gatewayProvider || this.config.verification.gatewayProvider,
      },
      fallback: {
    ...
    const needsReinitialization =
      ...
      JSON.stringify(oldConfig.routing) !== JSON.stringify(this.config.routing) ||
      JSON.stringify(oldConfig.verification) !== JSON.stringify(this.config.verification) ||
      JSON.stringify(oldConfig.ao) !== JSON.stringify(this.config.ao) ||
      JSON.stringify(oldConfig.telemetry) !== JSON.stringify(this.config.telemetry)
